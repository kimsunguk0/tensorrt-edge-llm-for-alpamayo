...
def resize_chw_image(image_chw, target_width, target_height):
    """Resize a CHW RGB image to the given output size."""
...
def resize_image_frames(image_frames, target_width, target_height):
    """
    Resize image frames from [Cam, T, C, H, W] to [Cam, T, C, target_h, target_w].
    """
    ...
    for cam_idx in range(num_cams):
        for step_idx in range(num_steps):
            resized[cam_idx, step_idx] = resize_chw_image(
                image_frames[cam_idx, step_idx], target_width, target_height
            )
    return resized
...
class LatestSampleStore:
    """
    Holds only the newest sample. New arrivals overwrite older unread samples.
    """
    ...
    def snapshot(self):
        with self._lock:
            if self._latest_sample is None:
                return None, 0
            return self._latest_sample, self._sequence
...
def make_sample_http_handler(sample_store, target_width, target_height):
    class SampleRequestHandler(BaseHTTPRequestHandler):
        def do_GET(self):
            if self.path == "/health":
                sample, sequence = sample_store.snapshot()
                payload = {
                    "status": "ok",
                ...
                if sample is not None:
                    payload["t0_us"] = int(sample["t0_us"])
                    payload["clip_id"] = str(sample["clip_id"])
...
def run_server_mode(grabber, server_host, server_port, max_frames, target_width, target_height, log_online_history_flag):
    sample_store = LatestSampleStore()
    ...
    print(f">> Jetson can pull /latest for NPZ payloads and /health for status")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--max_frames', type=int, default=100,
                        help='Maximum number of frames to collect. Use -1 for unlimited in server mode.')
